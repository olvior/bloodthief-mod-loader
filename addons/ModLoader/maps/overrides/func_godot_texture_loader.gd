...
func create_material(texture_name: String) -> Material:
	...
	var material_path: String = (
		"%s/%s.%s"
		% [map_settings.base_texture_dir, texture_name, map_settings.material_file_extension]
	)
	...
	for pbr_suffix in PBRSuffix.values():
		...
		if tex:
			if material is ShaderMaterial:
				material = StandardMaterial3D.new()
				material.set_texture(StandardMaterial3D.TEXTURE_ALBEDO, texture)
			var enable_prop: String = (
				PBR_SUFFIX_PROPERTIES[suffix] if suffix in PBR_SUFFIX_PROPERTIES else ""
			)
			if enable_prop != "":
